...
static int L2Sqr(const int16_t* a, const int16_t* b, size_t d) {
#ifdef USE_SSE
#ifdef USE_AVX
#ifdef USE_AVX512
    ...
    return d ? sum + L2SqrRef<int, int16_t>(a, b, d) : sum;
#else
    return L2SqrRef<int, int16_t>(a, b, d);
#endif
}
...
static inline int L2SqrST(const int16_t* x, const int16_t* y, size_t d) {
    return L2Sqr(x, y, d);
}
